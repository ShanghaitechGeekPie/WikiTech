...
以下以 Outlook 为例，给出配置指南：

1. 在 Outlook 中点击添加账户，输入个人邮箱地址，将自动检测配置

2. 如没有自动检测完成，请按如下信息填写：

> 发件：SMTP服务器：`smtp.shanghaitech.edu.cn` 加密方式：`SSL/TLS` 端口：`465`
> 
> 收件（二选一）：IMAP服务器：`imap.shanghaitech.edu.cn` 加密方式：`SSL/TLS` 端口：`993`
> 
> 收件（二选一）：POP3服务器：`pop.shanghaitech.edu.cn` 加密方式：`SSL/TLS` 端口：`995`

3. 密码请不要填写邮箱账户密码！你需要在 个人设置——安全设置——客户端专用密码 页面中生成专属密码，将他作为你的客户端填写的密码而不是邮箱密码。

更多客户端配置参考 [此页面](https://mail.shanghaitech.edu.cn/coremail/help/clientoption_zh_CN.jsp)，请注意，邮箱现在使用了多因素认证 + 客户端专属密码这一业界常用的方案来防止邮件被盗用，文档可能没有即使更新。

### 网页邮箱使用第三方认证器

由于新的规定，网页邮箱需要绑定：备用邮箱、短信和 Coremail APP 三选二的二次验证，三者都不太方便。

下面是一种不使用 Coremail APP，连接到第三方认证器（例如 Aegis、Microsoft Authenticator 等） OTP 应用的方法：
...
> **⚠️ 注意事项**
> 1. **千万不要忘记取卡**：离开前请务必点击屏幕上的“返回”或“退出”，确保账户已登出，防止余额被盗刷。
> 2. **卡放稳**：由于结算是在取卡时进行的，打印过程中请勿移动校园卡。
> 3. **报修/咨询**：如遇卡纸或扣费异常，可联系图信中心工作人员（Tel: 021-20684836）。
{.is-warning}
